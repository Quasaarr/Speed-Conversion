...
# Desired_Unit-Input Loop
    ...
    while True:
        try:
            round_number = int(input("How many decimal points would you like to round to? "))
            break
        except ValueError:
            print("Please input integers only!", line_break)

# Convert and Print Loop
    if unit.upper() == "M":
        if desired_unit.upper() == "K":
            converted_speed = speed * 1.6093442101
            print(f"You were going {converted_speed.__round__(round_number)}km/h!", line_break)
        if desired_unit.upper() == "KN":
            converted_speed = speed * 0.868976823
            print(f"You were going {converted_speed.__round__(round_number)} knots!", line_break)
        if unit.upper() == desired_unit.upper():
            print(f"You were going {speed.__round__(round_number)}mph!", line_break)

    if unit.upper() == "K":
        if desired_unit.upper() == "M":
            converted_speed = speed * 0.6213711111
            print(f"You were going {converted_speed.__round__(round_number)}mph!to fo", line_break)
        if desired_unit.upper() == "KN":
            converted_speed = speed * 0.5399569444
            print(f"You were going {converted_speed.__round__(round_number)} knots!", line_break)
        if unit.upper() == desired_unit.upper():
            print(f"You were going {speed.__round__(round_number)}km/h!", line_break)

    if unit.upper() == "KN":
        if desired_unit.upper() == "M":
            converted_speed = speed * 1.1507789973
            print(f"You were going {converted_speed.__round__(round_number)}mph!", line_break)
        if desired_unit.upper() == "K":
            converted_speed = speed * 0.0005144443
            print(f"You were going {converted_speed.__round__(round_number)}km/h!", line_break)
        if unit.upper() == desired_unit.upper():
            print(f"You were going {speed.__round__(round_number)} knots!", line_break)
